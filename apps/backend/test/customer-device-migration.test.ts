...
test('device migration preserves legacy credentials without guessing a client from its name', () => {
  ...
  try {
    db.exec('DROP INDEX customer_captures_owner_saved; ALTER TABLE customer_captures DROP COLUMN saved_via; ALTER TABLE customer_connections DROP COLUMN client_kind; PRAGMA user_version=12');
...
test('saving-platform migration retains existing content and original arrival dates', () => {
  ...
  let db = openDb(join(dir, 'test.db'));
  try {
    db.exec('DROP INDEX customer_captures_owner_saved; ALTER TABLE customer_captures DROP COLUMN saved_via; PRAGMA user_version=13');
