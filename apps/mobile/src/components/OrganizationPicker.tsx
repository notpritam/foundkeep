...
export function OrganizationPicker({ value, onChange, filter = false, manageFolders = true }: { value: OrganizationChoice; onChange: (value: OrganizationChoice) => void; filter?: boolean; manageFolders?: boolean }) {
  ...
  return <>
    ...
    <Modal visible={visible} presentationStyle="pageSheet" animationType={motion ? 'slide' : 'none'} onRequestClose={() => { if (!saving) setVisible(false); }}><Screen keyboard>
      ...
      <ScrollView contentContainerStyle={styles.page} keyboardShouldPersistTaps="handled">
        ...
        <Field label={filter ? 'Find a tag' : 'Create a tag'} value={tag} onChangeText={setTag} maxLength={40} autoCapitalize="none" returnKeyType="done" onSubmitEditing={() => addTag(tag)} placeholder="e.g. design ideas" />{tag.trim() ? <Button secondary label={filter ? 'Filter by tag' : 'Add tag'} onPress={() => addTag(tag)} /> : null}
