...
export default function SignIn() {
  ...
  const submit = async () => { setLoading(true); setError(''); try { await login({ email: email.trim(), password }); router.replace((consumePendingRoute() || '/(app)/collection') as Href); } catch (value) { setError((value as Error).message); } finally { setLoading(false); } };
