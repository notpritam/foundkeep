...
export default function Register() {
  ...
  return <Screen keyboard><ScrollView keyboardShouldPersistTaps="handled" contentContainerStyle={styles.page}><Brand compact /><View style={styles.heading}><Text style={typography.title}>Start your collection.</Text><Text style={typography.body}>A place for everything you want to keep.</Text></View><FrostedPanel style={styles.form}><OAuthButtons /><Field label="Name" value={name} onChangeText={setName} textContentType="name" autoCapitalize="words" returnKeyType="next" /><Field label="Email" value={email} onChangeText={setEmail} textContentType="emailAddress" keyboardType="email-address" autoCapitalize="none" autoCorrect={false} returnKeyType="next" /><Field label="Password" value={password} onChangeText={setPassword} textContentType="newPassword" secureTextEntry help="12 characters or more." returnKeyType="done" onSubmitEditing={submit} /><Message error>{error}</Message><Button label="Create account" onPress={submit} loading={loading} /><LegalFooter /><Pressable accessibilityRole="button" onPress={() => router.replace('/(auth)/sign-in')} style={styles.textButton}><Text style={styles.link}>Already have an account? Sign in</Text></Pressable></FrostedPanel></ScrollView></Screen>;
}
const styles = StyleSheet.create({ page: { flexGrow: 1, padding: 22, gap: 24 }, heading: { gap: 8, marginTop: 12 }, form: { gap: 16, marginHorizontal: -6 }, textButton: { minHeight: 44, alignItems: 'center', justifyContent: 'center' }, link: { color: colors.muted, fontSize: 13, textAlign: 'center' } });
