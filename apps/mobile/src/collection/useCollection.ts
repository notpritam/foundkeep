import { compareRecent, mergeRecentPage } from '../../../../packages/shared/src/collection-presentation.ts';
...
export function useCollection(filters: { q?: string; type?: string; batchId?: string; folderId?: string; tag?: string }) {
  ...
  const load = useCallback(async (mode: 'initial' | 'refresh' | 'more' | 'background' = 'initial') => {
    ...
    setError('');
    try {
      const result = await client.listCaptures({ ...query, cursor: mode === 'more' ? cursor.current || undefined : undefined }, { reload: mode !== 'more' });
      if (!focused.current || current !== sequence.current || key !== latestKey.current) return;
      setCaptures(previous => {
        if (mode !== 'more') return mergeRecentPage(previous, result.captures, mode === 'background' && expanded.current && !!result.nextCursor);
        const ids = new Set(previous.map(capture => capture.id));
        return [...previous, ...result.captures.filter(capture => !ids.has(capture.id))].sort(compareRecent);
      });
      if (mode !== 'background' || !expanded.current || !result.nextCursor) cursor.current = result.nextCursor;
      if (mode !== 'background') expanded.current = mode === 'more';
      failures.current = 0; nextPoll.current = Date.now() + (result.captures.some(item => item.status === 'pending' || item.status === 'processing') ? 5_000 : 15_000);
  ...
  useFocusEffect(useCallback(() => {
    ...
    const timer = setInterval(() => {
      if (AppState.currentState === 'active' && Date.now() >= nextPoll.current) void load('background');
    }, 5_000);
